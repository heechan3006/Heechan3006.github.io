...
- 아주 어려운 탐색문제입니다.
- 배양액을 배치할 수 있는 위치에 빨간색, 초록색 배양액을 배치하는데 최악의 경우에 O(10C5x5C5)일것으로 예상합니다.
- 배치된 배양액을 모두 채워서 꽃이 몇개나오는지 알기 위해서 bfs를 사용하면 시간복잡도가 O(NM)이기 때문에, 총 O(NxMx10C5) = 약 7.56 x 10^7 정도로 시간안에 해결이 가능할 것으로 보입니다.
